...
public class shuffle_the_array {
    ...
    public static int[] shuffle(int[] nums, int n) {
        int[] numsFirstHalf = Arrays.copyOf(nums, n);
        int[] numsSecondHalf = Arrays.copyOfRange(nums, n, nums.length);
        int index = 0;
        for (int i = 0; i < n; i++) {
            nums[index++] = numsFirstHalf[i];
            nums[index++] = numsSecondHalf[i];
        }
        return nums;
    }
