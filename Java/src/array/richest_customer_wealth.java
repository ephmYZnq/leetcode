...
public class richest_customer_wealth {
     ...
     * 
     * 示例 1：
     * 输入：accounts = [[1,2,3],[3,2,1]] 输出：6 
     * 解释：第 1 位客户的资产总量 = 1 + 2 + 3 = 6 第 2 位客户的资产总量 = 3 + 2 + 1 = 6 两位客户都是最富有的，资产总量都是 6 ，所以返回 6 。 
     * 
     * 示例 2：
     * 输入：accounts = [[1,5],[7,3],[3,5]] 输出：10 
     * 解释： 第 1 位客户的资产总量 = 6 第 2 位客户的资产总量 = 10 第 3 位客户的资产总量 = 8 第 2 位客户是最富有的，资产总量是 10 
     * 
     * 示例 3：
    ...
    public static Integer maximumWealth(int[][] accounts) {
        List<Integer> l = new ArrayList<Integer>();
        for (int[] i: accounts) {
            l.add(Arrays.stream(i).sum());
        }
        return l.stream().max(Comparator.naturalOrder()).orElse(0);
    }

    public static void main(String[] args) {
        int[][] nums =  new int[][]{{2,2,3},{1,8,3},{1,2,3}};
